...
1. **PoC budget violation.** Three systems on the critical path (Postgres source-of-truth, Pinecone vectors, OpenSearch lexical) cannot reach the US-01 walking skeleton in 8 hours with the resilience/observability work still to do. The interview asks for a Staff Engineer's PoC — a Staff Engineer demonstrates judgment about scope, not stamina.
...
3. **Conformance theater risk.** Choosing Pinecone+OpenSearch *because Netea uses them* — without the corpus scale that justifies it — is exactly the trap the task description warns against ("free to choose what fits best"). A staff engineer who picks the bigger system because it's what's familiar to the interviewer has chosen wrong.
...
**Why we kept it in the matrix**: this is the option we'd recommend if the interview pushback is "OK but at 1M scale your pgvector choice is wrong." Qdrant or Weaviate is the **honest scale-up answer**, ahead of OpenSearch, *if* the workload at 1M scale is primarily semantic retrieval. If the workload is genuinely hybrid at scale, OpenSearch wins. So Option D is the conditional runner-up.
...
- It is the **task spec's lead example** ("OpenSearch vs. Pinecone vs. Postgres"); we cannot credibly omit it.
...
Kept short-listed *despite the cut above* because the task explicitly mentions Pinecone and OpenSearch as their current stack. Eliminating it without naming it in the decision record would look like avoidance. It earns its slot in the short-list by being the *foil* against which Options A and B are defended.
...
3. **Demo cost.** The admin journey TUI mockup (`admin-ingests-batch.yaml` Step 1) shows a single CLI streaming per-question logs. A worker-process design either matches that UX by polling the worker (extra glue) or breaks it (Sam runs two terminals during the interview demo, which looks bad).

**If this cut is wrong**: T3 would win if we were demonstrating a *production* ingestion system rather than a PoC. The reviewer's likely question — "how would you scale this?" — gets the Expansion E Section 7 answer, not the T3-implemented-in-PoC answer.

### Cut: T4 — Real queue (SQS / Redis Streams)

**Where it looked attractive**: Production-ready.

**Why we cut it**: same as T3, more severe. AWS dependency for a take-home demo is wrong shape. Adds infra setup with zero PoC-scope benefit. Easy reject.

### Kept: T1 — Synchronous inline (the recommendation)

Discussed in `recommendation.md`. The taste argument: matches the TUI mockup in the journey artifact exactly. Maps the Expansion A Section 3 decision matrix to a single function. Zero infra. Maximally inspectable during the interview demo.
